...
object NativeHiddenFromObjCInheritanceChecker : DeclarationChecker {
    override fun check(declaration: KtDeclaration, descriptor: DeclarationDescriptor, context: DeclarationCheckerContext) {
        ...
        superClass?.let {
            val superClassIsHiddenFromObjC = checkClassIsHiddenFromObjC(it)
            if (superClassIsHiddenFromObjC) {
                // Align behavior with K2 and highlight just type, not constructor call.
                val superConstructorCall = (declaration as? KtClassOrObject)?.superTypeListEntries
                    ?.getOrNull(0) as? KtSuperTypeCallEntry
                val reportOn = superConstructorCall?.typeReference ?: declaration
                context.trace.report(ErrorsNative.INHERITS_FROM_HIDDEN_FROM_OBJC_CLASS.on(reportOn))
            }
        }
        val interfaces = descriptor.getSuperInterfaces()
        interfaces.forEachIndexed { index, classDescriptor ->
            val interfaceIsHiddenFromObjC = checkClassIsHiddenFromObjC(classDescriptor)
            if (interfaceIsHiddenFromObjC) {
                val reportOn = (declaration as? KtClassOrObject)?.superTypeListEntries?.getOrNull(index + 1) ?: declaration
                context.trace.report(ErrorsNative.IMPLEMENTS_HIDDEN_FROM_OBJC_INTERFACE.on(reportOn))
            }
        }
    }
}
